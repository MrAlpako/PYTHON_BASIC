...
class Icecream(Restoran):
    
    def __init__(self, r_name, r_kuhnya):
        super().__init__(r_name, r_kuhnya)
        self.flavors = []
        
    def sorta(self, *sort):
        self.sort = sort
        self.flavors.append(self.sort)
        print('Мороженное в наличии:')
        for i in self.sort:
            print('<0:',i)
        
ice=Icecream('Эшь', 'Вкусная')
...
ice.sorta('Орео','Сникерс','Пломбир')